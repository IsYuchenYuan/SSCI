...
class UNet2D(nn.Module):
    def __init__(self, in_chns, class_num, use_multi=False):
        super().__init__()
        ...
        self.up3 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.fuse = nn.Sequential(
            nn.Conv2d(in_channels=480,out_channels=256,kernel_size=3,padding=1),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(),
        ...
        if self.use_multi:
            self.finallayer = nn.Conv2d(256, 64, kernel_size=3, padding=1)
        else:
            self.finallayer = nn.Conv2d(16, 64, kernel_size=3, padding=1)
        self.cls = nn.Conv2d(64, class_num, kernel_size=3, padding=1)
