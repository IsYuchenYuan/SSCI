...
parser.add_argument('--exp', type=str, default='NNUNet_CL_fst_pseudo_norm_10000', help='model_name')
...
if __name__ == "__main__":
    ...
    past_teacher = create_model(ema=True)
    ...
    iter_num = 0
    # iter_num = torch.load(model_path)['epoch']
    max_epoch = max_iterations // len(l_trainloader) + 1
    lr_ = base_lr
    # bestloss = torch.load(model_path)['best_loss']

    def loss_back(noisy_labels):
        ...
        # CL loss
        # pretrian, no pseudo labels
        if noisy_labels == "None":
            weak_supervised_loss = torch.tensor(0)
        else:
            noisy_label_batch = noisy_labels
            with torch.no_grad():
                # use further teacher to generate prediction with no noise (ideally)
                ema_output_no_noise,_ = ema_model(volume_batch)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, (sampled_batch_l,sampled_batch_ul) in enumerate(zip(l_trainloader,ul_trainloader)):
            ...
            # generate the pseudo label using normal teacher model
            if iter_num >= 500:
                with torch.no_grad():
                    pseudo_labels,_ = past_teacher(volume_batch[labeled_bs:])
                    pseudo_labels = F.softmax(pseudo_labels,dim=1)
                    pseudo_labels = torch.argmax(pseudo_labels,dim=1)
                    noisy_labels = torch.cat((label_batch[:labeled_bs], pseudo_labels), dim=0)
            else:
                noisy_labels="None"
            ...
            for _ in range(3):
                ...
                optimizer.zero_grad()
                loss, _, _, _, _, _ = loss_back(noisy_labels)
                loss.backward()
                optimizer.step()
            ...
            optimizer.zero_grad()
            loss, loss_seg, loss_seg_dice, consistency_loss, weak_supervised_loss, consistency_weight = loss_back(noisy_labels)
            loss.backward()
            optimizer.step()
            update_ema_variables(model, past_teacher, args.ema_decay, iter_num)
            ...
            if dist.get_rank() == 0:
                if iter_num >= 2000 and iter_num % 100 == 0:
                    ...
                    if bestloss > testing_dice_loss:
                        ...
                        state = {'epoch': iter_num,  # 保存的当前轮数
                                 'state_dict': model.module.state_dict(),  # 训练好的参数
