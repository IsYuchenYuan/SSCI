...
from utils.losses import dice_loss,PixelPrototypeCELoss
from dataloaders.brainvessel import BrainVessel, RandomCrop, SizeCrop, CenterCrop, RandomRotFlip, ToTensor, \
    Normalization,Normalization_filter, Normalminmax_filter
...
parser.add_argument('--exp', type=str, default='vessel_multi', help='model_name')
...
parser.add_argument('--patchsize', type=list, default=[128,128,16],  help='whether use unlabel data')
...
proto_loss = PixelPrototypeCELoss()
criterion.to(device)
...
def validation(net, testloader):
    ...
    with torch.no_grad():
        for i, sampled_batch in enumerate(testloader):
            ...
            # loss_cls = 0.5 * loss_cls_2d + loss_cls_3d
            val_dice_loss = loss_cls_3d.item()
...
if __name__ == "__main__":
    ...
    # checkpoint = "../Ourmodel/Proto_3slice_sup_nointial/iter_1000.pth"
    net = net.to(device)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < 1000:
                ...
                loss_seg_dice = dice_loss(outputs_soft[:,1,...], label2d)
                loss_cls_2d = 0.5 * (loss_cls_ce + loss_seg_dice)

                loss_cls_ce_3d = criterion(cls_seg_3d, label_batch)
                outputs_soft_3d = F.softmax(cls_seg_3d, dim=1)
                loss_seg_dice_3d = dice_loss(outputs_soft_3d[:,1,...], label_batch)
